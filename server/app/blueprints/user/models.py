...
# ResourceMixin - custom class our model inherits from. Provides common
#   functionality to the model like:
#       Save function
#       Delete function
...
class User(UserMixin, ResourceMixin, db.Model):
    # Ordered dictionary - Remembers the order in which key-value pairs are
    # inserted.
    ROLE = OrderedDict([
        ('member', 'Member'),
        ('admin', 'Admin')
    ...
    # Table name to be used for this model
    __tablename__ = 'users'
    ...
    # Authentication
    # Enum set of possible string values which the column is contrainied to
    # :param: *Role: role names from ROLE OrderedDict
    # :param: name: the name of the type, required by postgres
    # :param: native_enum: Use the DB's native ENUM type when available;
        # False uses VARCHAR
    role = db.Column(db.Enum(*ROLE, name='role_types', native_enum=False),
                     index=True, nullable=False, server_default='member')

    # :param: server_default: Default text value for this column
    active = db.Column('is_active',  db.Boolean(), nullable=False,
                       server_default ='1')

    # :param: index: When True, indicates the column is indexed
    ...
    def __init__(self, **kwargs):
        # Flask-SQLAlchemy's constructor
        super(User, self).__init__(**kwargs)

        self.password = User.encrypt_password(kwargs.get('password', ''))

    # Classmethod - method belongs to the class and not an instance of it.
    @classmethod
